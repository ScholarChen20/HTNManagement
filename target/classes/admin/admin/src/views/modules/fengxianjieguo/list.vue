		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","fontSize":"inherit","textAlign":"right","display":"block","order":"2"}' >
					<div :style='{"margin":"0 0px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="结果" prop="jieguo">
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">结果</label>
						<el-select clearable v-model="searchForm.jieguo" placeholder="请选择结果" >
							<el-option v-for="(item,index) in jieguoOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
				...
				<el-row class="actions" :style='{"margin":"0px 0 20px","color":"#fff","flexWrap":"wrap","textAlign":"left","flexDirection":"row","display":"flex","width":"100%","fontSize":"inherit"}'>
					<el-button class="add" v-if="isAuth('fengxianjieguo','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						增加
					</el-button>
					<el-button class="del" v-if="isAuth('fengxianjieguo','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"0px solid #eee","width":"100%","padding":"0","margin":"20px 0 0","borderRadius":"0px","background":"rgba(255,255,255,.9)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						...
						label="结果时间">
						<template slot-scope="scope">
							{{scope.row.jieguoshijian}}
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('fengxianjieguo','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","height":"40px"}'></span>
								详情
							</el-button>
							<el-button class="edit" v-if=" isAuth('fengxianjieguo','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="del" v-if="isAuth('fengxianjieguo','删除') " type="primary" @click="deleteHandler(scope.row.id )">
...
import AddOrUpdate from "./add-or-update";
    ...
    init () {
          this.jieguoOptions = "低危风险,中危,高危".split(',')
    },
    search() {
      this.pageIndex = 1;
      this.getDataList();
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.jieguo!='' && this.searchForm.jieguo!=undefined){
            params['jieguo'] = this.searchForm.jieguo
          }
    params['sort'] = 'jieguoshijian';
    params['order'] = 'desc';
			let user = JSON.parse(this.$storage.getObj('userForm'))
			console.log(user)
			this.$http({
				url: "fengxianjieguo/page",
				method: "get",
				params: params
			}).then(({ data }) => {
				if (data && data.code === 0) {
	...
	fengxianjieguostatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'fengxianjieguo/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
