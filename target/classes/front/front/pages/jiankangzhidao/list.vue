		...
		<view :style='{"minHeight":"100vh","width":"100%","padding":"24rpx","position":"relative","background":"#eefcff","height":"auto"}'>
			<view class="cu-bar bg-white search" :style='{"width":"100%","padding":"8rpx 20rpx 8rpx 4rpx","borderRadius":"16rpx","background":"none","display":"flex","height":"auto"}'>
				...
				<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==0" class="search-form round">
					...
					<input :style='{"border":"0","padding":"8rpx 20rpx 8rpx 20rpx","color":"#333","borderRadius":"40rpx","background":"#fff","width":"100%","lineHeight":"72rpx","fontSize":"28rpx","height":"72rpx"}' v-model="searchForm.zhidaobiaoti" type="text" placeholder="指导标题" ></input>
				</view>
				<view :style='{"margin":"0 12rpx","flex":"1","position":"relative"}' v-if="queryIndex==1" class="search-form round">
					<text class="icon iconfont icon-fangdajing01" :style='{"color":"#a5ccc4","left":"0px","textAlign":"center","display":"none","width":"80rpx","fontSize":"40rpx","lineHeight":"68rpx","position":"absolute","right":"0px"}'></text>
					<input :style='{"border":"0","padding":"8rpx 20rpx 8rpx 20rpx","color":"#333","borderRadius":"40rpx","background":"#fff","width":"100%","lineHeight":"72rpx","fontSize":"28rpx","height":"72rpx"}' v-model="searchForm.zhidaoleixing" type="text" placeholder="指导类型" ></input>
				</view>
			...
			<view class="list" :style='{"width":"100%","padding":"0","margin":"40rpx 0 0","color":"#76c4d8","height":"auto"}'>
				<view @tap="onDetailTap(product)" class="listm flex flex-between" :style='{"boxShadow":"0 2rpx 12rpx #76c4d880","padding":"20rpx","margin":"0 0 40rpx","borderRadius":"60rpx","alignItems":"center","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","height":"auto"}' v-for="(product,index) in list" :key="index">
					...
					<view class="listmr" :style='{"width":"calc(100% - 180rpx)","padding":"0","margin":"0","height":"auto"}'>
						<view class="col3 f30 elip mb15" :style='{"padding":"0 0px","margin":"0","overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}'>{{product.zhidaobiaoti}}</view>
						<view class="col3 f30 elip mb15" :style='{"padding":"0 0px","margin":"0","overflow":"hidden","whiteSpace":"nowrap","color":"#333","width":"100%","lineHeight":"48rpx","fontSize":"28rpx","textOverflow":"ellipsis"}'>{{product.zhidaoleixing}}</view>
					...
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('jiankangzhidao','修改')) || (!userid && isAuthFront('jiankangzhidao','修改'))" @tap.stop.proevent="onUpdate" :data-row="product">
						...
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('jiankangzhidao','删除')) || (!userid && isAuthFront('jiankangzhidao','删除'))" @tap.stop.proevent="onDelete" :data-row="product">
					...
					<!-- #ifndef MP-WEIXIN -->
					<view :style='{"width":"100%","padding":"8rpx 20rpx","justifyContent":"space-between","display":"flex","height":"auto"}'>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('jiankangzhidao','修改')) || (!userid && isAuthFront('jiankangzhidao','修改'))" @tap.stop.proevent="onUpdateTap(product)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-edit"></text>
							<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>修改</text>
						</view>
						<view :style='{"display":"flex"}' v-if="(userid && isAuth('jiankangzhidao','删除')) || (!userid && isAuthFront('jiankangzhidao','删除'))" @tap.stop.proevent="onDeleteTap(product.id)">
							<text :style='{"margin":"0 8rpx 0 0","fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}' class="cuIcon-delete"></text>
							<text :style='{"fontSize":"28rpx","lineHeight":"1","color":"#666","display":"inline-block"}'>删除</text>
						</view>
					</view>
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.zhidaobiaoti){
					params['zhidaobiaoti'] = '%' + this.searchForm.zhidaobiaoti + '%'
				}
				if(this.searchForm.zhidaoleixing){
					params['zhidaoleixing'] = '%' + this.searchForm.zhidaoleixing + '%'
				}
				let user = uni.getStorageSync("appUserid")?JSON.parse(uni.getStorageSync('userSession')):{}
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`jiankangzhidao`, params);
                } else {
                    res = await this.$api.list(`jiankangzhidao`, params);
                }
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('jiankangzhidao', JSON.stringify([id]));
			...
			async search(){
				...
				if(this.searchForm.zhidaobiaoti){
					searchForm['zhidaobiaoti'] = '%' + this.searchForm.zhidaobiaoti + '%'
				}
				if(this.searchForm.zhidaoleixing){
					searchForm['zhidaoleixing'] = '%' + this.searchForm.zhidaoleixing + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`jiankangzhidao`, searchForm);
                } else {
                    res = await this.$api.list(`jiankangzhidao`, searchForm);
                }
				// 如果是第一页数据置空
