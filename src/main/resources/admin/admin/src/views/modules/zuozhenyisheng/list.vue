<template>
	<div class="main-content" :style='{"color":"#666","padding":"20px 30px 30px","fontSize":"14px"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit"}' :inline="true" :model="searchForm">
				<el-row :style='{"width":"100%","fontSize":"inherit","textAlign":"right","display":"block","order":"2"}' >
					...
					<div :style='{"margin":"0 0px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="科室" prop="keshi">
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('zuozhenyisheng','预约医生')" @click="yuyueyishengCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
		...
		<yuyueyisheng-cross-add-or-update v-if="yuyueyishengCrossAddOrUpdateFlag" :parent="this" ref="yuyueyishengCrossaddOrUpdate"></yuyueyisheng-cross-add-or-update>
...
import yuyueyishengCrossAddOrUpdate from "../yuyueyisheng/add-or-update";
	export default {
		data() {
			return {
				keshiOptions: [],
				...
				yuyueyishengCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			yuyueyishengCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.yuyueyishengCrossAddOrUpdateFlag = true;
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.yuyueyishengCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.yuyueyishengCrossaddOrUpdate.init(row.id,type);
				});
			},
